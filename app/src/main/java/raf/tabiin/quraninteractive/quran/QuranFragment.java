package raf.tabiin.quraninteractive.quran;
...
import raf.tabiin.quraninteractive.adapter.quran.AdaptersFirstUse;
import raf.tabiin.quraninteractive.adapter.quran.DrawerQuranAdapter;
import raf.tabiin.quraninteractive.adapter.quran.DrawerSuraContentAdapter;
import raf.tabiin.quraninteractive.databinding.FragmentQuranBinding;
...
public class QuranFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        binding.suraContent.addOnScrollListener(
                new RecyclerView.OnScrollListener() {
                    @Override
                    public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                        if (dy < 0 && !binding.fabQuranContent.isShown())
                            binding.fabQuranContent.show();
                        else if (dy > 0 && binding.fabQuranContent.isShown())
                            binding.fabQuranContent.hide();
                    }
                }
        );

        binding.fabQuranContent.setOnClickListener(v -> {
            openDrawerMaterial();
        });
    ...
    public void openDrawerMaterial() {
        if (!binding.quranContentLayout.isOpen()) {
            binding.quranContentLayout.openDrawer(Gravity.LEFT);
        } else {
            binding.quranContentLayout.closeDrawer(Gravity.LEFT);
        }
    }
